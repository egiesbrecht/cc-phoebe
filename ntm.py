...
from rnn_modeling import Output, RNNConfig


class NTMForParityCheck(nn.Module):
...
class NTM(nn.Module):
    ...
    def reset(self, batch_size=1):
        ...
        for i in range(self.num_heads):
            prev_read = torch.zeros([batch_size, self.memory.m])#.to("cuda")
            nn.init.kaiming_uniform_(prev_read)
            #prev_read = self.reads_fc(torch.Tensor([[0.]])).to("cuda")
...
class Controller(nn.Module):
    ...
    def forward(self, x, prev_reads):
        ...
        #print(x.shape, [n.shape for n in prev_reads])
        x = torch.cat([x, *prev_reads], dim=1)
        # Get hidden and cell states
        #self.h_state = self.h_state.to(x.device)
        #self.c_state = self.c_state.to(x.device)
        #print(x.device, self.h_state.device, self.c_state.device)
...
class Head(nn.Module):
    ...
    def _gated_interpolation(self, w, prev_w, g):
        ...
        '''
        #prev_w = prev_w.to(w.device)
        #print(w.device, prev_w.device, g.device)
...
class Memory(nn.Module):
    ...
    def content_addressing(self, key, beta):
        ...
        #self.memory = self.memory.to(key.device)
